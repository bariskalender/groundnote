...
class DocumentIndexingService:
    ...
    def index_document(
        ...
        *,
        ...
        run_token = self.indexing_registry.claim(document_id, pipeline_token=pipeline_token)
        try:
            chunks = self._prepare_for_indexing(document_id, force_reindex=force_reindex)
        except BaseException:
            self.indexing_registry.release(document_id, run_token)
            raise
        ...
        try:
            ...
            self.indexing_registry.complete(
                document_id,
                run_token,
                lambda: self._persist_completed_index(document_id, serialized, collector),
            )
        except BaseException:
            self._unload_embedding_model()
            try:
                self.indexing_registry.fail(
                    document_id,
                    run_token,
    ...
    def _persist_completed_index(
        self,
        document_id: str,
        serialized: list[tuple[str, SerializedEmbedding]],
        collector: IndexingMetricsCollector,
    ) -> None:
        """Persist vectors, FTS, integrity proof, and Ready status in one transaction."""
